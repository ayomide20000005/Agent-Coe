...
# Set to True to activate OCR processing. Keep False for safe deployment/testing.
OCR_ENABLED = False 
...
# --- FIXED OCR DOCUMENT FUNCTION (EasyOCR) ---
def extract_text_from_image(image_file):
    if not OCR_ENABLED:
        return "OCR ENGINE PAUSED. Set OCR_ENABLED = True in code to activate."
    
    try:
        img = Image.open(image_file)
        # Initialize Reader (Cached logic could be added here for speed, but simple for now)
        # gpu=False ensures compatibility with Streamlit Cloud
        reader = easyocr.Reader(['en'], gpu=False) 
        results = reader.readtext(img)
        
        # Extract text from the list of [bbox, text, confidence]
...
def calculate_verification_level(text, web_results):
    # If OCR is paused, text will be the pause message, handle gracefully
    if "PAUSED" in text:
        return "⏸️ PAUSED", "blue", 0, ["OCR Engine is currently disabled in code."]
        ...
        if st.button("Scan & Calculate Verification Level", type="primary"):
            with st.spinner("📄 Analyzing text, hashing, and cross-referencing web..."):
                ...
                if text is None or text.startswith("ERROR"):
                    ...
                    st.warning(f"⚠️ {text}")
                    st.info("To enable scanning: Open `app.py`, set `OCR_ENABLED = True`, and push to GitHub.")
                else:
                    lines = text.split('\n')
                    potential_queries = []
                    for line in lines:
